...
if 0 %this part randomly generate one  packet and verify service time of that packet
            clear all;
            Av_mean = 1; 
            Bu_mean = 4;
            s1= 3;
            ...
            lambda = 30;
            u = Bu_mean;
            v = -exp(-s1/Av_mean)*(Av_mean*(Av_mean+s1)/(Av_mean*(1-exp(-s1/Av_mean))))+Av_mean^2/(Av_mean*(1-exp(-s1/Av_mean)));
            check_each_part = 1;
            check_p = 1;
            p  = exp(-s1/Av_mean);
            a= 1;
            ...
            Scenario3 = ((v/(v+u)).*(1-p)).* (ES_AvArrive+s1+v);%insufficient available
            ...
            w = sort(w);
                ...
                long_sav_flag2 = 0;
                while (~send_flag )
                        slot_s1 = find(slot <= sst,1,'last'); slot_s2 = find(slot > sst,1,'first');
                        slot_e1 = find(slot < sst+s1,1,'last'); slot_e2 = find(slot >= sst+s1,1,'first');
                            ...
                            if(status_flag(i)== 3)
                             ...
                             if(status_flag(i)== 2)
                                if(P4(i)~=0)
                                    ...
                                    mean_first_u_busy(i) = (slot(first_slot4(i))-w(i));
                                end
                             end
...
if 1 %verify summation of   
    ...
    for k=1:nu
        su(k) = sum(uex_b([1:r(k)-1], k))+ sum(vex_b([1:r(k)-1], k));   %arrive at busy
    end
  
         vex_a = exprnd(v,max(r), N);
    ...
    a=1
    fprintf('E[ssv^2]: %1.3f  = %1.3f   \n', mean(ssv.^2),(2*(u^2+v^2+u*v)*((1/p)-a)+(((2-p)/p^2)-((1+2*a)/p)+a^2+a)*(u+v)^2)+(2*u^2)+2*u*((1/p)-a)*(u+v));
...
%  
     ...
     fprintf('var[ssv]: %1.3f  = %1.3f   \n',var(ssv),(((2*(u^2+v^2+u*v)*((1/p)-a)+(((2-p)/p^2)-((1+2*a)/p)+a^2+a)*(u+v)^2)+(2*u^2)+2*u*((1/p)-a)*(u+v))-((((1/p))*(u))+(((1/p)-1)*(v)))^2));
...
if 0
    ...
    vd = exprnd(v, [1, nsv]);
    wv = vd.*vx;
    channel = reshape([vd; ud], [1, nu+nsv]);
    slot1 = nan(size(channel));
    slot1(1) = 0;
    P= 0;
    P1 = 0;
    for j=1:length(slot1)
                slot1(j+1) = slot1(j) + channel(j);
    end
    chf = reshape([ zeros(1,nsv);ones(1,nu)], [1, nu+nsv]);
    chf = [chf,0];
    w(k) = rand(1)*slot1(end);
    a = vd-wv;
    a1 = a(vd-wv<s1);
    b = ud-wu;
        slot_s1 = find(slot1 <= w(k),1,'last');
        slot_s2 = find(slot1 > w(k),1,'first');
        if(chf(slot_s1)==0) % available
            c1(k) = slot1(slot_s2)-w(k);
            c4(k) = w(k)-slot1(slot_s1);
            c5(k) = slot1(slot_s2)- slot1(slot_s1);
            c8(k) = channel(slot_s1);
            channels_list1(k) = slot_s1;
            P = P+1;
            if(slot1(slot_s2)-w(k)<s1)
                c3(k) = slot1(slot_s2)-w(k);
                P = P+1;
            end
        
         
        elseif(chf(slot_s2-1)==1) % busy
            c2(k) = slot1(slot_s2)-w(k);
            c6(k) = w(k)-slot1(slot_s2-1);
            c7(k) = slot1(slot_s2)- slot1(slot_s2-1);
            c9(k) = channel(slot_s2-1);    
            channels_list2(k) = slot_s1;
            P1 =P1+1;
        else
            pause;
        end 
    ...
    c1 = c1(c1~=0);c2 = c2(c2~=0);c3 = c3(c3~=0);c4 = c4(c4~=0);c5 = c5(c5~=0);c6 = c6(c6~=0);c7 = c7(c7~=0);c8 = c8(c8~=0);c9 = c9(c9~=0);
    v1 = -exp(-s1/v)*(v*(v+s1)/(v*(1-exp(-s1/v))))+v^2/(v*(1-exp(-s1/v)));
    v = v/2;
    v2 = -exp(-s1/v)*(v*(v+s1)/(v*(1-exp(-s1/v))))+v^2/(v*(1-exp(-s1/v)));
    %fprintf('mean fav method 1:%f \nmean fav method 1 <s1: %f \nAnalytical mean fav method 1 <s1: %f \nmean fb method 1 %f \nmean fav method 2: %f \nmean fb method 2:%f \nmean fav method 2 <s1: %f \nAnalytical mean fav method 2 <s1: %f \n',mean(a),mean(a1),v2,mean(b),mean(c1),mean(c2),mean(c3),v1);
    fprintf('Busy %f other half: %f channel: %f \navailable %f other half: %f channel: %f \n',mean(c2),mean(c6),mean(c7),mean(c1),mean(c4),mean(c5));
    mean(c8),mean(c9)
    toc
end    