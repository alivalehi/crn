...
if sim.Framing_mode == 1
    ...
    if ~isempty(sim.ch.Rch) && (sim.ch.BER >=0) && (sim.ch.BER <= 0.5) && (sim.coding.impBERD >=1) && (sim.coding.impBERH >= 1) && (sim.coding.CRD <= 1) && (sim.coding.CRD >= 1/8) && (sim.coding.CRH <= 1) && (sim.coding.CRH >= 1/8) 
        ...
        sim.Plen_av = sim.T_av * sim.lambda * (sim.N / sim.CRD) + (sim.H / sim.CRH) + sim.nTbitsD + sim.nTbitsH; %Expected average packet length for each T
...
%         if isempty(sim.ch.Rch0) 
%             output_traffic = sim.Plen_min ./ (sim.Kmin ./ sim.lambda);  %on average at eeach k ./ sim.lambda, there is one packet to send
%             sim.ch.Rch0 = 2 * output_traffic; %Channel rate is adjusted for K > KinP_PB: Lower K might result in unstable queue
%         end
        ...
        if ~isfield(sim.CRN,'Ch_AvailMeanTime')
            sim.CRN.Ch_AvailMeanTime = [];
        end
        ...
        sim.CRN.Ch_NotAvailMeanTime = sim.CRN.chNotAvRatio * sim.Plenv/sim.ch.Rch;
%a=sim.CRN.Ch_AvailMeanTime, pause; b=sim.CRN.Ch_NotAvailMeanTime,        pause;
...
if isfield(sim.run, 'quick_run_coef')  %adjustments for quick run
    if ~isempty(sim.run.quick_run_coef)
        sim.run.Nsym = sim.run.quick_run_coef*sim.run.Nsym;     
        sim.run.minSym = sim.run.quick_run_coef*sim.run.minSym; 
        sim.run.maxSym = sim.run.quick_run_coef*sim.run.maxSym; 
        sim.run.minPack = sim.run.quick_run_coef*sim.run.minPack; 
        sim.run.maxPack = sim.run.quick_run_coef*sim.run.maxPack;
        end
end

if ~ispc %for server enforce regardless of the parameters
