function [new_tp, kp, fiv, tindex] = f_perform_framing(Framing_mode, x, TK, debug_active)  %excluded lambda and NinP
...
%a=1/lambda;
[SUn,Nt] = size(x);
t = zeros(SUn,Nt);   %arrivals of Poisson process
fiv = zeros(SUn,Nt);  %distance of symbols from end of a packet period
 

t(:,1) = x(:,1);
for j = 1:SUn,  
for i = 1:Nt-1,  
    t(j,i+1) = t(j,i) + x(j,i+1); %calc arrival times from interarrival times
end
end
...
if Framing_mode == 1
    ...
    if debug_active,    fprintf('T/2:%1.6f      Framing delay:%1.6f  Deviation:%1.6f percent \n', Tf/2, mean (fiv), 200*( mean(fiv)-Tf/2)/Tf ); end
     ...
     fiv(j,:) = reshape(fi ,1, K * Kp);
     end 
     merged_tp = reshape(tp',1,[]);
     [new_tp,tindex] = sort(merged_tp);
