...
if sim.control.simulation
    ...
    for SU = sim.SU
        ...
        for nruns = 1:NUM_RUNS   %parfor
            ...
            u_rnd =sim.u;%datasample([sim.umin:0.1:sim.umax],sim.Nexperts); %rand(1,sim.Nexperts)*10;
            v_rnd = sim.v;%datasample([sim.vmin:0.1:sim.vmax],sim.Nexperts);%rand(1,sim.Nexperts)*10;sim.v .*ones(1,sim.Nexperts);
            ...
            while(~LastSymbol_flag)
                ...
                if(length(sim.Av_length)>sim.windowsshift)
                    sim.Av_length = mle(sim.Av_length,'distribution','exp');
                    sim.B_length = mle(sim.B_length,'distribution','exp');
                      k_t = [sim.k_min:sim.k_max];
                    [EE_prediction_vector,P_s,perr] = EE_Analisor(k_t,1,sim,sim.B_length,sim.Av_length,sim.ch.BER,1);
                    [xx,k_suggest]= min(EE_prediction_vector);
                EE_track(end+1) = xx;
                k_track(end+1) = k_suggest;
                end
...
%                     [EE_prediction_vector,P_s,perr] = EE_Analisor(k,1,sim,u(1),v(1),sim.ch.BER,0);
...
%                 segment = segment+1;
          ...
          %          delete 'C:\Temp\matlab\alldata.mat';
                ...
                save('C:\Temp\matlab\alldataML.mat','-regexp', '^(?!(t|int_arrivals|t_t|t_t_copy)$).');
                %   if(segment>25)
%                 if(sim.variable == 0)%was0
%                     close all;
%                     plot(-cumsum(EE_real_track-EE_real));
%                     hold on;
%                     plot(cumsum(EE_real));
...
%                     %
                    if(segment == 2)
                        ...
                        KPRED11(1:sim.n) = .05+(k.*ones(1,sim.n));
                    else
                        %                    KANAL = [KANAL ,datasample(Kanallll,sim.windowsshift,'Replace',false) ];
                        lastindex = length(KPRED11);
                        for wq=1:sim.windowsshift
                            KPRED11(lastindex+wq) = .05+(k);%.05+(k_last.*ones(1,sim.windowsshift));
                        end
                        
                    end
                    
                    kanal = sim.kanal;
                    Kanalplot = kanal(1:1420000);
                    kanal = [];
