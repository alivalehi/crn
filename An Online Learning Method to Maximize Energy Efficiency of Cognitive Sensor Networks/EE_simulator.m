...
length(kv);
   Av_length = sim.Av_length;
    B_length = sim.B_length;
%tic
for j=1: length(kv)
    %   if(mod(j,100)==0)
    % j
    ...
    while (~r_flag)   %try for all retransmission
        ...
        while (~send_flag )
            ...
            elseif (cht_s1+1 == cht_s2) && (cht_e1+1 == cht_e2) && (cht_s1 == cht_e1) && (chf(cht_e2) == 1)
                ...
                sim.lastsent = sst;
                Av_length(end+1) = cht(cht_s2) - cht(cht_s1);
                B_length(end+1) = cht(cht_s1) - cht(cht_s1-1);
                N3(r) = N3(r)+1;
            ...
            elseif (cht_s1+1 == cht_s2) && (cht_e1+1 == cht_e2) && (chf(cht_s1) == 1)
                N2(r) = N2(r)+1;
                B_length(end+1) = cht(cht_s2) - cht(cht_s1);
...
sim.Av_length = unique(Av_length);
sim.B_length = unique(B_length);
end
