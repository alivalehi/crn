...
nPop=sim.Nexperts;        % Population Size
...
pop=repmat(empty_individual,nPop,1);
                    ...
                    BestSol=EE_real_vector(Best_prediction);
...
for it=1:MaxIt
    ...
    for i=1:nPop
        ...
        beta=unifrnd(beta_min,beta_max,2);
        y_u_rnd = u_rnd(a)+beta(1).*(u_rnd(b)-u_rnd(c));
        y_v_rnd = v_rnd(a)+beta(2).*(v_rnd(b)-v_rnd(c));
   %     y =pop(a).Position+beta.*(pop(b).Position-pop(c).Position);
        y_u_rnd = max(y_u_rnd, sim.umin);
        y_v_rnd = max(y_v_rnd, sim.vmin);
		y_u_rnd = min(y_u_rnd, sim.umax);
        y_v_rnd = min(y_v_rnd, sim.vmax);
		y = [y_u_rnd y_v_rnd]; 
        % Crossover
        z=zeros(size(x));
        j0=randi(2);
       ...
       % NewSol.Position=z;
      [NewSol,P_s,perr]= EE_Analisor(k,1,sim,squeeze(z(:,1))',squeeze(z(:,2))',BER_rnd,0);
...
%    disp(['Iteration ' num2str(it) ': Best Cost = ' num2str(BestCost(it))]);
  ...
  k_t = [sim.k_min:sim.k_max];
                    [EE_prediction_vector1,P_s,perr] = EE_Analisor(k_t,Best_prediction,sim,u_rnd(best_i),v_rnd(best_i),BER_rnd(end),1);
        ...
        elseif(Ev_method == 2)
             ...
             %   BER_rnd = datasample(Berr,sim.Nexperts);%Berr(randperm(length(Berr),sim.Nexperts));
              ...
              %  fprintf('segment :%d u:%f v:%f BER:%f k:%f\n',segment,upredict(end),vpredict(end),BERpredict(end),k);
                ...
                save('C:\Temp\matlab\alldatamix.mat','-regexp', '^(?!(t|int_arrivals|t_t|t_t_copy)$).');
         %   if(segment>25)
