...
if sim.control.simulation
    ...
    for SU = sim.SU
        ...
        for nruns = 1:NUM_RUNS   %parfor
            ...
            Berr = [5e-5:1e-6:5e-4 5e-5:1e-6:5e-4 5e-5:1e-6:5e-4];
          ...
          %  BER_rnd = ones(1,sim.Nexperts).*sim.ch.BER;
            ...
            while(~LastSymbol_flag)
                ...
                EE_prediction_vector_track(ll) = EE_prediction_vector(Best_prediction);
               ...
               % if(ll<round(1*sim.Itlimit))
                kp  = round((1-sim.omega)*mode(Kpredict)+(sim.omega)*k_suggest);
                [EE_prediction_vector5,P_s,perr] = EE_Analisor(kp,Best_prediction,sim,upredict(end),vpredict(end),BERpredict(end),1);
                [EE_prediction_vector6,P_s,perr] = EE_Analisor(kp+1,Best_prediction,sim,upredict(end),vpredict(end),BERpredict(end),1);
                [EE_prediction_vector4,P_s,perr] = EE_Analisor(kp-1,Best_prediction,sim,upredict(end),vpredict(end),BERpredict(end),1);
                kres = [kp-1 kp kp+1];
                [eeres,keeres] = min([EE_prediction_vector4,EE_prediction_vector5,EE_prediction_vector6]);
                kpredict_paleo(ll) = kp;
              %  k = kres(keeres);
...
%                 end
                ...
                Kpredict(ll) = kres(keeres);
                %   xx(ll) = xx;
                length(t_t);
  fprintf('iteration :%d u:%f v:%f BER:%f k:%f \n',ll,upredict(end),vpredict(end),BERpredict(end),keeres);
